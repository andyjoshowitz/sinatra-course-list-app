...
class SemestersController < ApplicationController
  ...
  post '/semesters/new' do
    if user = User.find_by_id(session[:id])
      if user.semesters.build(season: params[:season], year: params[:year])
  ...
  get '/semesters/:id' do
    ...
    if user && user.semesters.include?(@semester)
      erb :'/semesters/show'
    else
      redirect to '/login'
    end
  ...
  patch '/semesters/:id' do
    ...
    @semester = Semester.find_by_id(params[:id])
    if user && user.semesters.include?(@semester)
      @semester = Semester.find_by_id(params[:id]).update(season: params[:season], year: params[:year])
      erb :'semesters/show'
    else
      redirect to '/login'
    end
